...
| `interpolate(start, end, progress)` | per-key object lerp used by `useSpringTrigger` scrub mode |

> [!warning] `interpolate()` parses at most **one number per value**
> A `SpringTrigger` `from`/`to` value like `transform: "translate3d(0, 18%, 0)"`
> (or any compound transform string) is mangled — only the first number lerps,
> the frames come out malformed, and react-spring throws *"The arity of each
> output value must be equal"*. Use react-spring shorthand keys instead:
> `{ y: "18%" }`, `{ rotate: "8deg" }`, `{ scale: 0.9 }` — one number each,
> which both this util and react-spring handle correctly.
