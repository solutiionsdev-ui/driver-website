...
- **Scale down** (1280px < viewport ≤ 1920px) — `vw`-based
  `html { font-size }` media queries in `globals.css`. At each breakpoint's
  design base width the root font-size resolves to 16px; between breakpoints
  it tracks the viewport.
- **Hold** (viewport ≤ 1279px) — below `xl` the page is the stacked
  adaptation rather than a scaled copy of a wider frame, so the root
  font-size holds at `FONT_BASE` (16px) and the breakpoints do the adapting.
  That range carries `baseWidth: null` in `grid.config.ts` — see
...
> [!note] `SiteHeader` removed 2026-07-26
> The fixed navigation was deleted when the page was reduced to the bare hero
> effect (see [[changelog]]); recoverable from git history. `LazyCookie` was
> also unmounted from the root layout at the same time (component retained).
>
> **Superseded 2026-08-04:** the page is no longer the bare effect — it is the
> GRIDO1 hero. It has a masthead again, but as a feature-local
> `views/home/sections/hero/hero-nav.tsx`, **not** a restored `SiteHeader`.
> There is still no global navigation and `components/ui/` is still empty.
